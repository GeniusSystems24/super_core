---
name: super-core-migration-v2-to-v2.1
description: >
  Step-by-step guide to migrate Flutter code from super_core v2.0.x to v2.1.0 —
  the release that removes the last static token values from the design system.
  In v2.0 brand tokens moved to the dynamic `SuperTokensData`, but each field
  still had a `SuperTokensData.default*` compile-time constant and `SuperMarker`
  exposed a `defaultColor`. v2.1.0 removes ALL of those: tokens are now read
  exclusively from the ambient theme. Use this whenever you upgrade super_core
  (or a dependent) from 2.0.x to 2.1.0.
---
